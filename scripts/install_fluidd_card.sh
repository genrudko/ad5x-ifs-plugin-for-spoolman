...
for FILE in "$CARD_SOURCE" "$LAYOUT_SOURCE" "$VISIBILITY_SOURCE"; do
    [ -f "$FILE" ] || {
...
for PATTERN in card layout visibility; do
    for FILE in "$FLUIDD_DIR"/ifs-spoolman-$PATTERN*.js; do
        [ -e "$FILE" ] || continue
        case "$FILE" in
            "$CARD_TARGET"|"$LAYOUT_TARGET"|"$VISIBILITY_TARGET") continue ;;
        esac
        rm -f "$FILE"
    done
done
