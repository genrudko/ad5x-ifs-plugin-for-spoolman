  ...
  function colorVisual(single, multiple, direction) {
    ...
    if (colors.length === 1) {
      return {
        ...
        count: 1,
        sequential: false,
        dark: relativeLuminance(colors[0]) < 0.055
      };
    }
    ...
    return {
      ...
      count: colors.length,
      sequential,
      dark: colors.every(color => relativeLuminance(color) < 0.055)
    };
  }
  ...
  function renderSlot(slotData) {
    ...
    if (provider !== "spoolman") {
      ...
      const warning = slotData.mismatch
        ...
        : "";
      const swatchClass = visual.dark ? "combined-swatch is-dark" : "combined-swatch";

      body = `<div class="combined-card"><div class="${swatchClass}" title="${esc(tooltip)}" style="--spool-visual:${esc(visual.swatch)}"></div><div class="combined-main"><div class="combined-topline"><div class="combined-name" title="${esc(inventory.name || `Катушка #${inventory.spool_id}`)}">${esc(inventory.name || `Катушка #${inventory.spool_id}`)}</div><div class="combined-vendor" title="${esc(inventory.vendor || "Без производителя")}">${esc(inventory.vendor || "Без производителя")}</div></div><div class="combined-detail"><div class="combined-meta"><span class="combined-pill">ID ${esc(inventory.spool_id)}</span><span class="combined-pill">${esc(inventory.material || "Материал не указан")}</span>${colorCount}</div><div class="combined-weight">Остаток: ${formatWeight(remaining)} / ${formatWeight(initial)}${percent === null ? "" : ` · ${Math.round(percent)}%`}</div></div>${percent === null ? "" : `<div class="combined-progress" title="Остаток ${Math.round(percent)}%"><span style="width:${percent.toFixed(1)}%;background:${esc(visual.progress)}"></span></div>`}${warning}</div></div>`;
    }
  ...
  async function openDialog(slot) {
    ...
    try {
      ...
      const render = () => {
        ...
        const options = [{ id: "", name: "Не назначено", vendor: "Снять привязку Spoolman", material: "", visual: { swatch: "#475569", count: 1, colors: ["#475569"], dark: false }, remaining: null }, ...available.map(spoolInfo)];
        list.innerHTML = options.map(item => `<label class="assign-option${String(item.id) === selected ? " selected" : ""}"><input class="assign-radio" type="radio" name="assign-spool" value="${esc(item.id)}"${String(item.id) === selected ? " checked" : ""}><span class="assign-swatch${item.visual.dark ? " is-dark" : ""}" title="${esc(item.visual.colors.join(", "))}" style="--spool-visual:${esc(item.visual.swatch)}"></span><span class="assign-main"><span class="assign-name" title="${esc(item.name)}">${esc(item.name)}</span><span class="assign-meta">${esc(item.vendor)}${item.material ? ` · ${esc(item.material)} · ID ${esc(item.id)}${item.visual.count > 1 ? ` · ${esc(colorCountText(item.visual.count))}` : ""}` : ""}</span></span><span class="assign-weight">${item.id === "" ? "" : formatWeight(item.remaining)}</span></label>`).join("") || `<div class="assign-empty">Подходящих катушек не найдено.</div>`;
