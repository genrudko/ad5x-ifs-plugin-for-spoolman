(() => {
  ...
  function sharpGradient(colors) {
    if (colors.length <= 1) return colors[0];
    const segment = 100 / colors.length;
    const stops = [];
    colors.forEach((color, index) => {
      stops.push(`${color} ${(segment * index).toFixed(2)}%`);
      stops.push(`${color} ${(segment * (index + 1)).toFixed(2)}%`);
    });
  ...
  function colorVisual(single, multiple, direction) {
    ...
    if (colors.length === 1) {
      return { colors, swatch: colors[0], progress: colors[0], count: 1 };
    }
    const swatch = sequential
      ? `conic-gradient(from 0deg, ${[...colors, colors[0]].join(", ")})`
      : `conic-gradient(from -90deg, ${colors.map((color, index) => {
          const start = (100 / colors.length * index).toFixed(2);
          const end = (100 / colors.length * (index + 1)).toFixed(2);
          return `${color} ${start}% ${end}%`;
        }).join(", ")})`;
    return { colors, swatch, progress: sharpGradient(colors), count: colors.length };
  }
  ...
  function renderSlot(slotData) {
    ...
    if (provider !== "spoolman") {
      ...
      body = `<div class="combined-card"><div class="combined-swatch" title="${esc(tooltip)}" style="background:${esc(visual.swatch)}"></div><div class="combined-main"><div class="combined-topline"><div class="combined-name" title="${esc(inventory.name || `Катушка #${inventory.spool_id}`)}">${esc(inventory.name || `Катушка #${inventory.spool_id}`)}</div><div class="combined-vendor" title="${esc(inventory.vendor || "Без производителя")}">${esc(inventory.vendor || "Без производителя")}</div></div><div class="combined-detail"><div class="combined-meta"><span class="combined-pill">ID ${esc(inventory.spool_id)}</span><span class="combined-pill">${esc(inventory.material || "Материал не указан")}</span>${visual.count > 1 ? `<span class="combined-color-count" title="${esc(tooltip)}">${esc(colorCountText(visual.count))}</span>` : ""}</div><div class="combined-weight">Остаток: ${formatWeight(remaining)} / ${formatWeight(initial)}${percent === null ? "" : ` · ${Math.round(percent)}%`}</div></div>${percent === null ? "" : `<div class="combined-progress"><span style="width:${percent.toFixed(1)}%;background:${esc(visual.progress)}"></span></div>`}${slotData.mismatch ? `<div class="combined-warning">Локальные параметры слота отличаются от данных Spoolman</div>` : ""}</div></div>`;
    }
  ...
  async function openDialog(slot) {
    ...
    try {
      ...
      const render = () => {
        ...
        list.innerHTML = options.map(item => `<label class="assign-option${String(item.id) === selected ? " selected" : ""}"><input class="assign-radio" type="radio" name="assign-spool" value="${esc(item.id)}"${String(item.id) === selected ? " checked" : ""}><span class="assign-swatch" title="${esc(item.visual.colors.join(", "))}" style="background:${esc(item.visual.swatch)}"></span><span class="assign-main"><span class="assign-name" title="${esc(item.name)}">${esc(item.name)}</span><span class="assign-meta">${esc(item.vendor)}${item.material ? ` · ${esc(item.material)} · ID ${esc(item.id)}${item.visual.count > 1 ? ` · ${esc(colorCountText(item.visual.count))}` : ""}` : ""}</span></span><span class="assign-weight">${item.id === "" ? "" : formatWeight(item.remaining)}</span></label>`).join("") || `<div class="assign-empty">Подходящих катушек не найдено.</div>`;
